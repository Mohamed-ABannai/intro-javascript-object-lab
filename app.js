...
2. Choose a value for "difficulty" that you think fits the game. Ex: "Easy", 
"Med" or "Hard". How would you assign it?
...
3. Access this pokemon that you want as your starter pokemon in 
the pokemon array and add them to the game.party array
...
for (let i = 0; i < game.party.length; i++) {
  ...
  console.log('Name:' + game.party[i].name)
...
for (let i = 0; i < pokemon.length; i++) {
  ...
  if (pokemon[i].starter) console.log(`Name: ${pokemon[i].name} and starter= ${pokemon[i].starter}`)
...
The time has come to make it so that we cannot catch a Pokemon 
when we do not have any pokeballs to catch it with. 
...
/*
Exercise 20
Copy the `catchPokemon` method that you just wrote above,
and paste it below. Modify is so that you can just pass
in the name of a Pokemon instead of an entire object,
and the method will look up the Pokemon from the data set for you.

The string passed in should be allowed to be any case
(for example, if the string 'PiKacHU' is passed to the function,
